...
object Database:
  ...
  /** Crea un transactor HikariCP configurado para usar el schema `config.schema`.
    *
    * El `connectionInitSql` establece `search_path` en cada conexión nueva del pool,
    * de modo que todas las queries de Doobie operan sobre el schema correcto
    * sin necesidad de calificar los nombres de tabla.
    *
    * @param config Configuración de base de datos.
    * @return Resource que libera el pool al finalizar.
    */
  def makeTransactor(config: DbConfig): Resource[IO, HikariTransactor[IO]] =
    for
      ...
      _ <- Resource.eval(
             xa.configure(ds =>
               IO(ds.setConnectionInitSql(s"SET search_path TO ${config.schema}"))
             )
           )
    yield xa
  ...
  /** Ejecuta las migraciones Flyway de forma síncrona al iniciar.
    ...
    *   1. Crear el schema si no existe (`CREATE SCHEMA IF NOT EXISTS portafolio`)
    *   2. Almacenar `flyway_schema_history` dentro de ese schema
    *   3. Establecer `search_path` durante la ejecución de cada migración
    *
    * Los scripts SQL en `db/migration/` no necesitan calificar los nombres
    * de tabla con el schema porque Flyway gestiona el `search_path`.
    */
