...
object WsMessage:
  ...
  given Encoder[WsMessage] = Encoder.instance {
    case PreviewUpdate(t, id, d) =>
      Json.obj("type" -> "preview_update".asJson, "entityType" -> t.asJson,
               "entityId" -> id.asJson, "data" -> d)
...
/** Ruta WebSocket para previsualización en tiempo real.
  ...
  * no soportan headers personalizados en el handshake inicial.
  */
object PreviewWebSocket:
  ...
  def routes(
    ...
    HttpRoutes.of[IO] {
      ...
      case GET -> Root / "ws" / "preview" :? TokenParam(token) =>
